...
namespace CMU462 {
	namespace DynamicScene {
		...
		void Mesh::linearBlendSkinning(bool useCapsuleRadius) {
			...
			//this->mesh.verticesBegin(); // ??????????????????????? danielstuff
			//LBSInfo

			// for every vertex v in __??:
			
			// find where this vertex would end up (in world coors) if it were transformed along with bone j
			//		find the cloesest point on joint j's bone segment (axis) and compute the distance to this closest point
        ...
        double wsum = 0;
				...
				for (int k = 0; k < skeleton->joints.size(); k++) {
					...
					Vector4D last_col = j->getBindTransformation().column(3);
					Vector3D j_thing = Vector3D(last_col.x, last_col.y, last_col.z);
					Vector3D pos_v_relative_to_j = v->bindPosition - j_thing; // v->position or v->bindPosition?  
...
=======
          double dist_ij = (pos_v_relative_to_j - pt_closest).norm();
					if (useCapsuleRadius && dist_ij > j->capsuleRadius)
							continue;
>>>>>>> d45d69c4de3e8c5bf670de884a6611d4e5f399d8
